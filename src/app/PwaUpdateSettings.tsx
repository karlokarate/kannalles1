...
export function PwaUpdateSettings({ pwa }: { pwa: PwaUpdateViewModel }) {
  const updateAvailable = pwa.phase === 'update-available' || pwa.phase === 'applying';
  ...
  return (
    <section className="settings-card settings-card--wide app-update-settings" aria-labelledby="app-update-settings-title" data-testid="pwa-update-settings">
      ...
      <dl className="app-update-details">
        <div><dt>Installierte App</dt><dd>v{pwa.currentAppVersion}</dd></div>
      ...
      <p className="settings-note">Beim Öffnen, bei der Rückkehr in die App, nach erneuter Internetverbindung und stündlich wird die bereitgestellte Version ohne Browsercache geprüft. Ein Update wird nie mitten in einer Eingabe erzwungen.</p>
      <p className="settings-note"><strong>Lokale Daten bleiben erhalten:</strong> Verlauf, Favoriten, eigene Produkte, Fotos, Einstellungen und persönliche Einheitskalibrierungen werden beim App-Update nicht gelöscht.</p>
      <div className="button-row">
        {updateAvailable && <button type="button" className="button button--primary" onClick={() => { void pwa.applyUpdate(); }} disabled={pwa.phase === 'applying'} data-testid="pwa-update-settings-apply">{pwa.phase === 'applying' ? 'Update wird aktiviert …' : 'Jetzt aktualisieren'}</button>}
