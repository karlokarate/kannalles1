...
import { CATALOG_SEARCH_PAGE_SIZE } from '../lib/catalog/catalogPagination';
    ...
    <section className="screen calculator-screen" aria-labelledby="calculator-title">
      ...
      {!c.manualMode ? <>
        <search><form className="search-card" onSubmit={submit} data-search-phase={c.search.phase}>
          ...
          <small>Die Produktsuche bleibt vollständig lokal. Mehrere Produkte ohne eigene Einheit werden als Portion verstanden; fehlt eine belastbare Portionsgröße, fragt die App sie produktspezifisch ab. Beispiel: „ein halbes Brötchen mit Nutella“.</small>
        ...
        {(c.search.phase === 'needs_product_choice' || (c.search.phase === 'resolved' && c.search.candidates.length > 1)) && <section className="results-panel" aria-labelledby="results-title" data-testid="catalog-search-results" data-result-count={c.search.candidates.length} data-page-size={CATALOG_SEARCH_PAGE_SIZE} data-page-number={c.searchPage + 1} data-order-authority="local-then-sqlite">
          <div className="section-title-row"><div><span className="eyebrow">Lokale Treffer vor SQLite-Reihenfolge</span><h2 id="results-title">{c.search.phase === 'resolved' ? 'Weitere passende Produkte' : 'Produkt auswählen'}</h2></div><span>{c.search.candidates.length} Treffer auf dieser Seite</span></div>
          <div className="result-list">{c.search.candidates.map((hit, index) => {
            ...
            return <button key={`${hit.productId}-${hit.resultIndex}`} type="button" className="product-result" data-testid="catalog-search-result" data-result-index={hit.resultIndex} data-rank-ordinal={hit.rankOrdinal} data-auto-select-eligible={String(auto.eligible)} data-catalog-eligible={String(eligible)} onClick={() => chooseCandidate(hit)}>
              <span className="result-position">{c.searchPage * CATALOG_SEARCH_PAGE_SIZE + index + 1}</span><ProductImage src={c.settings.productImageMode === 'remote' ? c.catalogPhotoUrl(hit.code) ?? catalogProductImageUrl(hit) : null} alt={hit.displayName} compact /><span className="result-copy"><strong>{hit.displayName}</strong>{isClinicCatalogProduct(hit) ? <ClinicTag compact /> : <small>{subtitle(hit.brand, hit.code) || 'Ohne Markenangabe'}</small>}</span><span className="result-nutrition">{nutritionLabel(hit)}</span>
            </button>;
          })}</div><nav className="pagination" aria-label="Weitere passende Produkte"><button type="button" className="button button--secondary" disabled={c.searchPage === 0} onClick={() => c.changeSearchPage(c.searchPage - 1)}>← Zurück</button><span>Seite {c.searchPage + 1} · {CATALOG_SEARCH_PAGE_SIZE} Treffer pro Seite</span><button type="button" className="button button--secondary" disabled={!c.searchHasNext} onClick={() => c.changeSearchPage(c.searchPage + 1)}>Weiter →</button></nav>
        </section>}
      ...
      </> : <ManualProductEditor c={c} diabetesPanel={diabetesPanel} />}
