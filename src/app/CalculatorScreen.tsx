...
import { isManualCatalogProduct } from '../lib/manualCatalog';
...
function positive(value: string, fallback: number): number { const parsed = Number(value); return Number.isFinite(parsed) && parsed > 0 ? parsed : fallback; }
  ...
  return <>
    <article className="meal-summary" data-testid="meal-summary">
      ...
      <div className="meal-item-list">{c.mealItems.map((item) => <article className="meal-item" key={item.id} data-testid="meal-item">
        ...
        <label className="field meal-item__amount" htmlFor={`meal-item-amount-${item.id}`}><span>Menge</span><QuantityStepper value={item.request.amount} min={0.01} max={10_000} onChange={(amount) => c.updateMealItem(item.id, amount)} ariaLabel={`${item.product.displayName}: Menge`} testId={`meal-item-amount-${item.id}`} /></label>
...
export function CalculatorScreen({ c }: { c: CatalogController }) {
  ...
  useEffect(() => { if (product && !isGenericCatalogProduct(product) && !isManualCatalogProduct(product) && (!isClinicCatalogProduct(product) || product.clinic.directCarbohydratesPerUnit === null)) setCalibrationOpen(!hasDefinedServing); }, [hasDefinedServing, product]);
  ...
  return (
    <section className="screen calculator-screen" aria-labelledby="calculator-title">
      ...
      {!c.manualMode ? <>
        ...
        {(c.search.phase === 'needs_product_choice' || (c.search.phase === 'resolved' && c.search.candidates.length > 1)) && <section className="results-panel" aria-labelledby="results-title" data-testid="catalog-search-results" data-result-count={c.search.candidates.length} data-order-authority="local-then-sqlite">
          <div className="section-title-row"><div><span className="eyebrow">Lokale Treffer vor SQLite-Reihenfolge</span><h2 id="results-title">{c.search.phase === 'resolved' ? 'Weitere passende Produkte' : 'Produkt auswählen'}</h2></div><span>{c.search.candidates.length} Treffer</span></div>
          <div className="result-list">{c.search.candidates.map((hit, index) => {
            ...
            return <button key={`${hit.productId}-${hit.resultIndex}`} type="button" className="product-result" data-testid="catalog-search-result" data-result-index={hit.resultIndex} data-rank-ordinal={hit.rankOrdinal} data-auto-select-eligible={String(auto.eligible)} data-catalog-eligible={String(eligible)} onClick={() => chooseCandidate(hit)}>
        ...
        {product && resolution && <article ref={productCardRef} tabIndex={-1} className="product-card" data-testid="catalog-product" data-product-id={product.productId} data-gtin={product.code} data-amount={c.request.amount} data-carbs-per-100-g={product.nutrition.basis === 'mass' ? product.nutrition.carbohydratesPer100 : ''} data-carbs-per-100-ml={product.nutrition.basis === 'volume' ? product.nutrition.carbohydratesPer100 : ''} data-nutrition-basis={product.nutrition.basis} data-unit-resolution-status={resolution.status}>
          ...
          <div className="calculation-grid">
            {c.search.candidates.length > 1 && <label className="field field--wide variant-picker"><span>Produktvariante</span><select value={String(product.productId)} onChange={(event: ChangeEvent<HTMLSelectElement>) => { const next = c.search.candidates.find((candidate) => String(candidate.productId) === event.target.value); if (next) chooseCandidate(next); }} data-testid="catalog-variant-select">{c.search.candidates.map((candidate) => <option key={candidate.productId} value={candidate.productId}>{candidate.displayName}{candidate.brand ? ` · ${candidate.brand}` : ''}</option>)}</select><small>Die Menge bleibt erhalten; Serving und Einheiten werden für die gewählte Variante neu aufgelöst.</small></label>}
            <label className="field" htmlFor="catalog-amount-input"><span>Menge</span><QuantityStepper value={c.request.amount} min={0.01} max={10_000} onChange={(amount) => c.setRequest((current) => ({ ...current, amount }))} ariaLabel={`${product.displayName}: Menge`} testId="catalog-amount-input" />{selected && (selected.unit === 'g' || selected.unit === 'ml') && <div className="amount-slider"><input type="range" min="1" max={selected.unit === 'g' ? '400' : '1000'} step="1" value={Math.max(1, Math.min(selected.unit === 'g' ? 400 : 1000, c.request.amount))} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setRequest((current) => ({ ...current, amount: Number(event.target.value) }))} aria-label={`${selected.unit === 'g' ? 'Gramm' : 'Milliliter'}-Menge per Schieberegler`} data-testid="catalog-amount-slider" /><small><span>1 {selected.unit}</span><span>{selected.unit === 'g' ? '400 g' : '1.000 ml'}</span></small></div>}</label>
          ...
          {product.nutrition.basis === 'mass' && !isGenericCatalogProduct(product) && !isManualCatalogProduct(product) && (!isClinicCatalogProduct(product) || product.clinic.directCarbohydratesPerUnit === null) && <details className="calibration-card" open={calibrationOpen} onToggle={(event) => setCalibrationOpen(event.currentTarget.open)} data-testid="catalog-calibration" data-status="always-available">
            ...
            <div className="calibration-fields"><label className="field"><span>Einheit</span><select value={c.calibrationUnit} onChange={(event: ChangeEvent<HTMLSelectElement>) => c.changeCalibrationUnit(event.target.value as typeof c.calibrationUnit)} data-testid="catalog-calibration-unit"><option value="piece">Stück</option><option value="bar">Riegel</option><option value="slice">Scheibe</option><option value="portion">Portion</option></select></label><label className="field" htmlFor="catalog-calibration-count"><span>Anzahl gemeinsam gewogen</span><QuantityStepper value={positive(c.calibrationCount, 10)} min={1} max={10_000} inputStep={1} onChange={(amount) => c.setCalibrationCount(String(Math.trunc(amount)))} ariaLabel="Anzahl gemeinsam gewogen" testId="catalog-calibration-count" /></label><label className="field"><span>Gesamtgewicht in g</span><input type="number" min="0.01" step="any" value={c.calibrationWeight} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setCalibrationWeight(event.target.value)} placeholder="z. B. 28,8" data-testid="catalog-calibration-weight" /></label></div>
        ...
        <div className="manual-grid"><label className="field field--wide"><span>Bezeichnung</span><input value={c.manual.label} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setManual((m) => ({ ...m, label: event.target.value }))} data-testid="manual-product-label" /></label><label className="field"><span>KH pro 100 {c.manual.basis === 'mass' ? 'g' : 'ml'}</span><input type="number" min="0" step="any" value={c.manual.carbohydratesPer100} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setManual((m) => ({ ...m, carbohydratesPer100: event.target.value }))} data-testid="manual-product-carbs" /></label><label className="field" htmlFor="manual-product-amount"><span>Menge in {c.manual.basis === 'mass' ? 'g' : 'ml'}</span><QuantityStepper value={positive(c.manual.amount, 100)} min={0.01} max={10_000} onChange={(amount) => c.setManual((m) => ({ ...m, amount: String(amount) }))} ariaLabel={`Menge in ${c.manual.basis === 'mass' ? 'Gramm' : 'Milliliter'}`} testId="manual-product-amount" /></label><label className="field"><span>Bezugsart</span><select value={c.manual.basis} onChange={(event: ChangeEvent<HTMLSelectElement>) => c.setManual((m) => ({ ...m, basis: event.target.value as 'mass' | 'volume' }))}><option value="mass">Gewicht</option><option value="volume">Volumen</option></select></label></div>
