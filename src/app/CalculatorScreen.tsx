  ...
  return <>
    <article className="meal-summary" data-testid="meal-summary">
      ...
      <div className="meal-item-list">{c.mealItems.map((item) => <article className="meal-item" key={item.id} data-testid="meal-item">
        ...
        <label className="field meal-item__amount"><span>Menge</span><QuantityStepper value={item.request.amount} min={0.01} max={10_000} onChange={(amount) => c.updateMealItem(item.id, amount)} ariaLabel={`${item.product.displayName}: Menge`} testId={`meal-item-amount-${item.id}`} /></label>
      ...
      {!c.manualMode ? <>
        ...
        {product && resolution && <article ref={productCardRef} tabIndex={-1} className="product-card" data-testid="catalog-product" data-product-id={product.productId} data-gtin={product.code} data-amount={c.request.amount} data-carbs-per-100-g={product.nutrition.basis === 'mass' ? product.nutrition.carbohydratesPer100 : ''} data-carbs-per-100-ml={product.nutrition.basis === 'volume' ? product.nutrition.carbohydratesPer100 : ''} data-nutrition-basis={product.nutrition.basis} data-unit-resolution-status={resolution.status}>
          ...
          <div className="calculation-grid">
            ...
            <label className="field"><span>Menge</span><QuantityStepper value={c.request.amount} min={0.01} max={10_000} onChange={(amount) => c.setRequest((current) => ({ ...current, amount }))} ariaLabel={`${product.displayName}: Menge`} testId="catalog-amount-input" />{selected && (selected.unit === 'g' || selected.unit === 'ml') && <div className="amount-slider"><input type="range" min="1" max={selected.unit === 'g' ? '400' : '1000'} step="1" value={Math.max(1, Math.min(selected.unit === 'g' ? 400 : 1000, c.request.amount))} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setRequest((current) => ({ ...current, amount: Number(event.target.value) }))} aria-label={`${selected.unit === 'g' ? 'Gramm' : 'Milliliter'}-Menge per Schieberegler`} data-testid="catalog-amount-slider" /><small><span>1 {selected.unit}</span><span>{selected.unit === 'g' ? '400 g' : '1.000 ml'}</span></small></div>}</label>
          ...
          {product.nutrition.basis === 'mass' && !isGenericCatalogProduct(product) && !isManualCatalogProduct(product) && (!isClinicCatalogProduct(product) || product.clinic.directCarbohydratesPerUnit === null) && <details className="calibration-card" open={calibrationOpen} onToggle={(event) => setCalibrationOpen(event.currentTarget.open)} data-testid="catalog-calibration" data-status="always-available">
            ...
            <div className="calibration-fields"><label className="field"><span>Einheit</span><select value={c.calibrationUnit} onChange={(event: ChangeEvent<HTMLSelectElement>) => c.changeCalibrationUnit(event.target.value as typeof c.calibrationUnit)} data-testid="catalog-calibration-unit"><option value="piece">Stück</option><option value="bar">Riegel</option><option value="slice">Scheibe</option><option value="portion">Portion</option></select></label><label className="field"><span>Anzahl gemeinsam gewogen</span><QuantityStepper value={positive(c.calibrationCount, 10)} min={1} max={10_000} inputStep={1} onChange={(amount) => c.setCalibrationCount(String(Math.trunc(amount)))} ariaLabel="Anzahl gemeinsam gewogen" testId="catalog-calibration-count" /></label><label className="field"><span>Gesamtgewicht in g</span><input type="number" min="0.01" step="any" value={c.calibrationWeight} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setCalibrationWeight(event.target.value)} placeholder="z. B. 28,8" data-testid="catalog-calibration-weight" /></label></div>
        ...
        <div className="manual-grid"><label className="field field--wide"><span>Bezeichnung</span><input value={c.manual.label} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setManual((m) => ({ ...m, label: event.target.value }))} data-testid="manual-product-label" /></label><label className="field"><span>KH pro 100 {c.manual.basis === 'mass' ? 'g' : 'ml'}</span><input type="number" min="0" step="any" value={c.manual.carbohydratesPer100} onChange={(event: ChangeEvent<HTMLInputElement>) => c.setManual((m) => ({ ...m, carbohydratesPer100: event.target.value }))} data-testid="manual-product-carbs" /></label><label className="field"><span>Menge in {c.manual.basis === 'mass' ? 'g' : 'ml'}</span><QuantityStepper value={positive(c.manual.amount, 100)} min={0.01} max={10_000} onChange={(amount) => c.setManual((m) => ({ ...m, amount: String(amount) }))} ariaLabel={`Menge in ${c.manual.basis === 'mass' ? 'Gramm' : 'Milliliter'}`} testId="manual-product-amount" /></label><label className="field"><span>Bezugsart</span><select value={c.manual.basis} onChange={(event: ChangeEvent<HTMLSelectElement>) => c.setManual((m) => ({ ...m, basis: event.target.value as 'mass' | 'volume' }))}><option value="mass">Gewicht</option><option value="volume">Volumen</option></select></label></div>
