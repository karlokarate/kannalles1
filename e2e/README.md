...
These tests exercise the built PWA, the production SQLite file, the shipped SQLite-WASM module and the OPFS SAH-pool. They never replace `catalogClient`, the worker or SQLite with a mock. The rollback case only corrupts the same-origin catalog transport after a valid slot has been installed, so validation and rollback still execute in production code.
...
The browser matrix is intentionally serial because an OPFS SAH-pool with the same directory name cannot be active concurrently in multiple browsing contexts on one origin.
